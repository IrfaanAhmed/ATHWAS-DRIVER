 ...
 */
class SequenceStep(context: Context?, attrs: AttributeSet?) : TableRow(context, attrs) {
    ...
    init {
        ...
        val attributes = getContext().theme.obtainStyledAttributes(attrs, R.styleable.SequenceStep, 0, R.style.SequenceStep)
    ...
    private fun setupAnchor(attributes: TypedArray) {
        if (!attributes.hasValue(R.styleable.SequenceStep_anchor1)) {
            anchor.visibility = View.INVISIBLE
        } else {
            setAnchor(attributes.getString(R.styleable.SequenceStep_anchor1))
        }
    }
